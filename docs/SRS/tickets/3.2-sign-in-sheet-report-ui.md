...
#### Schema Registration
- [ ] Add `signInSheetReportSchema` to `packages/shared-validators/src/schemas/report.ts`:
  ```ts
  ...
  ```
  - **Validation**: Add a `.refine()` (or equivalent) so that when `scope === 'jurisdiction'`, `cityTown` is required; otherwise runtime and types can diverge. The frontend form must enforce the same rule (disable submit or show validation error when scope is jurisdiction and cityTown is empty).
- [ ] Add to `generateReportSchema` discriminated union.
- [ ] Add to `REPORT_TYPE_MAPPINGS` in `reportTypeMapping.ts`:
  ```ts
  signInSheet: { databaseValue: 'SignInSheet', filename: 'signInSheet' }
  ```

#### Data Fetching
- [ ] Create `fetchSignInSheetData()` in `apps/report-server/src/committeeMappingHelpers.ts`:
...
#### PDF Component
- [ ] Create `apps/report-server/src/components/SignInSheet.tsx`:
...
- [ ] Follow the same React → HTML → PDF pattern used by `CommitteeReport.tsx` and `DesignatingPetition.tsx`.

#### Handler
- [ ] Add `signInSheet` case to `processJob()` in `apps/report-server/src/index.ts`:
  ```ts
...
#### Report Picker
- [ ] Update `apps/frontend/src/components/reports/GenerateReportGrid.tsx`:
...
#### Report Form Page
- [ ] Create `apps/frontend/src/app/sign-in-sheet-reports/page.tsx` — server component with auth check.
- [ ] Create `apps/frontend/src/app/sign-in-sheet-reports/SignInSheetForm.tsx` — client component:
...
- [ ] After submission, show `ReportStatusTracker` component for real-time job status via Ably.

#### API Route
- [ ] Update `/api/generateReport` handler to accept `type: 'signInSheet'` and forward scope/jurisdiction params.
- [ ] Apply jurisdiction scoping: if Leader, verify their `UserJurisdiction` includes the requested cityTown/legDistrict.

### Testing
- [ ] Report-server: Test `fetchSignInSheetData()` returns committees filtered by scope/jurisdiction.
- [ ] Report-server: Test `SignInSheet.tsx` renders correct HTML structure (member rows + blank rows + header).
- [ ] Report-server: Test `signInSheet` handler in `processJob()` produces a PDF file.
- [ ] Frontend: Test form validation (scope + cityTown required for jurisdiction scope).
- [ ] Frontend: Test Leader cannot select "Countywide" scope.
