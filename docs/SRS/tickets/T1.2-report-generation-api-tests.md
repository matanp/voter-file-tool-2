...
- [ ] Session exists but `session.user.id` is falsy (null/undefined) → 500 with structured error
- [ ] Verify no report record is created and no PDF API call is made

### Gap 2 — HMAC/gzip signing correctness (low severity)

- [ ] Verify the request sent to the PDF server includes the `x-webhook-signature` header
- [ ] Verify the request body is gzip-compressed (Content-Encoding or body is a Buffer, not plain JSON)
- [ ] Note: full cryptographic HMAC verification may be better suited to integration tests; unit test can assert the header is present and non-empty with a valid `WEBHOOK_SECRET` set
