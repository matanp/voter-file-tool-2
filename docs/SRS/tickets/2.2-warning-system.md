...
- [ ] Define a typed warning contract in the eligibility layer (stringly-typed warnings are not allowed):
...
- [ ] Warning generation never flips `eligible` to `false` by itself.

### Warning Rules

- [ ] **Possibly inactive voter (`POSSIBLY_INACTIVE`)**
...
- [ ] **Recent resignation (`RECENT_RESIGNATION`)**
...
- [ ] **Pending submission in another LTED (`PENDING_IN_ANOTHER_COMMITTEE`)**
  - Warning when voter has `CommitteeMembership.status=SUBMITTED` in another `committeeListId` for the same active term.

### API Integration

- [ ] Include warnings in successful responses for:
  - `POST /api/committee/add`
  - `POST /api/committee/requestAdd`
  - `POST /api/committee/handleRequest` (accept flow)
- [ ] Preserve current hard-stop behavior from 2.1:
  - Hard stops => reject (422)
  - Warnings only => proceed (200/201)
- [ ] Extend success response shape with `warnings?: EligibilityWarning[]`.

### Persistence + Audit

- [ ] Persist warning snapshot when a membership record is created or status-transitioned due to add/request/accept:
...
- [ ] Add warning context to audit metadata on relevant events (`MEMBER_SUBMITTED`, `MEMBER_ACTIVATED`) as `metadata.eligibilityWarnings`.
...
- [ ] `apps/frontend/src/app/committees/requests/RequestCard.tsx` (admin review):
  - If warning metadata exists on the pending request, display it before Accept/Reject.

### Optional Preflight Endpoint (recommended)

- [ ] Add read-only endpoint for pre-submit checks:
  - `GET /api/committee/eligibility?voterRecordId=...&committeeListId=...`
  - Returns full `{ eligible, hardStops, warnings }`
- [ ] Use this endpoint only for UX prefetch; mutation endpoints remain authoritative.

### Tests

- [ ] Unit tests for warning derivation helper(s):
...
- [ ] Route tests (`add`, `requestAdd`, `handleRequest`):
