...
- [ ] In `POST /api/committee/add`: Before connect, check if voter has `committeeId` set to a different committee; if so, return 400 with structured error
- [ ] In `POST /api/committee/requestAdd`: Same check before creating CommitteeRequest
- [ ] In `POST /api/committee/handleRequest` (accept flow): Same check before connecting — replacement requests handle remove-then-add; ensure add path rejects if voter is in another committee and this is not a replacement
- [ ] Add/update tests in `add.test.ts`, `requestAdd.test.ts`; add or update `handleRequest.test.ts` if not yet present
