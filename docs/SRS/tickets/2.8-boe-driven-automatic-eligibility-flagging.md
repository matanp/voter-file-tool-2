...
> Follow-up: Ticket 4.6 extends `EligibilityFlagStatus` with `RESOLVED_BY_RESCAN` for stale auto-resolution on recurring re-scans.
...
- [x] Create `POST /api/admin/eligibility-flags/run`:
  ...
  - Returns run summary (`scanned`, `newFlags`, `existingPending`, `autoResolved`, `durationMs`).
