...
- [x] Extend `committee/remove` (or add new endpoint `committee/resign`) to accept `action: "RESIGN"` in addition to generic remove behavior:
...
- [x] Auth: Admin-only operation
- [x] Zod validation: `resignationDateReceived` must be a valid date string; `resignationMethod` must be `EMAIL` or `MAIL`
- [x] Return updated membership record (or `{ success: true }`) on success
- [x] 404 if membership not found; 400 if membership is not currently `ACTIVE` (cannot resign a non-active member)

### Audit Logging

- [x] Log `MEMBER_RESIGNED` to `AuditLog` via `logAuditEvent` after successful status transition:
...
- [x] If ticket 1.5 (Audit Trail Infrastructure) is not yet complete: add `// TODO(1.5): log MEMBER_RESIGNED` comment in code and skip the call

### Admin UI

- [x] Committee member card/roster row: add "Record Resignation" action (button or dropdown item), distinct from "Remove"
- [x] Resignation form (modal or inline): fields for `resignationDateReceived` (date picker), `resignationMethod` (select: Email / Mail), and optional notes
- [x] Form validation: date and method are required before submit
- [x] On successful submit: member card updates to show `status=RESIGNED`; member is no longer shown in the active roster

### Seat Availability

- [x] After resignation, `assignNextAvailableSeat` (implemented in ticket 1.4) naturally returns the freed seat number for the next activation
- [x] No explicit seat clearing needed on the `Seat` model — availability is computed dynamically by checking which seat numbers have no active `CommitteeMembership`
