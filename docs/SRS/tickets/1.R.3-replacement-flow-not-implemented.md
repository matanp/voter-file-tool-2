...
- [ ] On accept, when `membership.submissionMetadata?.removeMemberId` is present:
...
- [ ] Run both transitions in a transaction for atomicity.
- [ ] If remove-target is not found or not ACTIVE, return 422 with explicit error (e.g. "Replacement target not found or no longer active").
- [ ] Update T1.1 tests to assert replacement behavior: accept with `removeMemberId` → target REMOVED, add-member ACTIVE.
- [ ] If replacement was intentionally deferred: update T1.1 ticket and UI copy to reflect "planned" rather than "done."
