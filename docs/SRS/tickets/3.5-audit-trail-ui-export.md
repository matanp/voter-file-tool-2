...
- [ ] `GET /api/admin/audit/export` — Export filtered results.
  ...
  - **CSV columns**: Timestamp, User Name, User Email, User Role, Action, Record Type, Record ID, Summary.
    - `Record Type` carries the display label (e.g. "Committee membership"), not the raw
      `entityType` value, matching the existing treatment of the `Action` column. Unknown
...
- [ ] Implement a horizontal filter bar at the top of the page:
  ...
  - **Record type filter** — Dropdown driven by `AUDIT_ENTITY_TYPE_OPTIONS` (label + description per option). The registry lists only entity types some code path actually writes; see [P2-audit-report-generation-not-logged.md](P2-audit-report-generation-not-logged.md) for the `Report` / `CommitteeTerm` gap that keeps them out of it.
...
| Record type | 120px | Human-readable label via `formatEntityTypeLabel`           |
| Summary     | flex  | Auto-generated from action + record type + key changes     |

> **As built:** the raw `entityType` string is no longer surfaced in the table — it is
> mapped to a display label through `AUDIT_ENTITY_TYPE_OPTIONS`. The record ID moved to
> the detail drawer only (copyable), so the table has no Entity ID column.
...
- [ ] Detail view contents:
  ...
  - **Record info**: "Record type" label + option description + full record ID (copyable).
