...
- [x] New enums (add to `schema.prisma`):
  - `MembershipStatus`: `SUBMITTED`, `REJECTED`, `CONFIRMED`, `ACTIVE`, `RESIGNED`, `REMOVED`, `PETITIONED_LOST`, `PETITIONED_TIE`
  - `RemovalReason`: `PARTY_CHANGE`, `MOVED_OUT_OF_DISTRICT`, `INACTIVE_REGISTRATION`, `DECEASED`, `OTHER`
  - `ResignationMethod`: `EMAIL`, `MAIL`
- [x] Canonical petition winner representation: `ACTIVE` with `membershipType=PETITIONED` (no dedicated `PETITIONED_WON` status)
