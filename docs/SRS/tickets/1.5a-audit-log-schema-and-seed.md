...
- [x] New `AuditAction` enum in `schema.prisma` per [SRS_DATA_MODEL_CHANGES.md](../SRS_DATA_MODEL_CHANGES.md) §1:
  ```prisma
  enum AuditAction {
    MEMBER_SUBMITTED
    MEMBER_REJECTED
...
- [x] New `AuditLog` model in `schema.prisma` per [SRS_DATA_MODEL_CHANGES.md](../SRS_DATA_MODEL_CHANGES.md) §2:
  ```prisma
  model AuditLog {
...
- [x] Add `auditLogs AuditLog[] @relation("AuditLogs")` to the `User` model

### Migration

- [x] Run `pnpm db_migrate` to generate and apply the Prisma migration
- [x] Verify migration creates the `AuditAction` enum, `AuditLog` table with all columns and indexes, and updates the `User` table with the relation

### SYSTEM User Seed

- [x] In `apps/frontend/prisma/seed.ts`, add a re-entrant upsert for the SYSTEM user:
  ```typescript
  ...
  ```
- [x] The SYSTEM user is required because `AuditLog.userId` is a non-nullable FK — a bare string sentinel would violate referential integrity
- [x] Upsert is re-entrant: running `pnpm db_seed` multiple times must not fail

### Validation

- [x] `npx prisma validate` passes
- [x] `npx prisma generate` produces updated client types
- [x] Seed runs successfully against a fresh and existing database
