...
- [ ] Add `MeetingType` enum to `apps/frontend/prisma/schema.prisma`:
...
- [ ] Replace placeholder `meetingRecordId String?` on `CommitteeMembership` with a real relation:
...
- [ ] Create `POST /api/admin/meetings`:
  - Admin-only.
  - Body: `{ meetingDate, meetingType?, notes? }`.
  - Creates `MeetingRecord`.
  - Logs `MEETING_CREATED` audit event.

- [ ] Create `GET /api/admin/meetings`:
  - Admin-only.
  - Returns paginated meeting list with counts by decision outcome.

- [ ] Create `GET /api/admin/meetings/[meetingId]/submissions`:
  - Admin-only.
...
- [ ] Create `POST /api/admin/meetings/[meetingId]/decisions`:
  - Admin-only.
  - Body:
...
- [ ] Validation and error handling:
  ...
  - 422 if any target membership is not `SUBMITTED`.
  - Per-membership capacity guard before activation; if full, decision must fail with explicit reason.

### Existing Routes Compatibility

- [ ] Keep `/api/committee/handleRequest` for one-off handling.
- [ ] Add optional `meetingRecordId` support on accept/reject path when called from meeting UI.
- [ ] Do not remove `/committees/requests` in this ticket.

### Admin UI

- [ ] Enable `/admin/meetings` in `apps/frontend/src/config/adminNav.ts` (`enabled: true`).
- [ ] Add page and components:
...
- [ ] Workflow:
  ...
  - Require rejection note input UI for rejected rows (optional in API, recommended in UI).
- [ ] On `/committees/requests`, add clear CTA linking to `/admin/meetings` for bulk processing.

### Audit Logging

- [ ] `MEETING_CREATED` on meeting creation.
- [ ] `MEMBER_CONFIRMED` and `MEMBER_ACTIVATED` on confirm.
- [ ] `MEMBER_REJECTED` on reject.
- [ ] Include `metadata.meetingRecordId` in all membership decision audit events.

### Tests

- [ ] Prisma/unit tests for schema relation validity and migration.
- [ ] API tests:
  ...
  - non-`SUBMITTED` membership rejected with 422
  - capacity collision returns deterministic error
- [ ] UI behavior tests for selection/bulk actions (at least one integration test for meetings page).
