...
- [ ] Reuse `CommitteeMembership` for petition candidates and outcomes.
- [ ] Add `petitionSeatNumber Int?` to `CommitteeMembership` to tie a candidate/outcome to a specific seat.
- [ ] Keep existing petition fields:
  - `petitionVoteCount`
  - `petitionPrimaryDate`
- [ ] Add index for petition lookups:
...
- [ ] Each petition workflow operation is scoped to one `(committeeListId, termId, seatNumber)`.
- [ ] Candidate rows for that seat are represented by `CommitteeMembership` records with:
...
- [ ] Create `POST /api/admin/petition-outcomes/record` (Admin-only):
...
- [ ] Create `GET /api/admin/petition-outcomes` (Admin-only):
  - Filter by term, committee, outcome status.
  - Returns seat-level outcome history.

### UI

- [ ] Enable `/admin/petition-outcomes` in `apps/frontend/src/config/adminNav.ts`.
...
- [ ] Log `PETITION_RECORDED` once per submission payload with metadata:
  - `committeeListId`, `termId`, `seatNumber`, candidate outcomes summary.
- [ ] For winner activation/loser transitions, keep membership transition audit events (`MEMBER_ACTIVATED`, etc.) consistent with status changes.

### Tests

- [ ] API tests for outcome validation:
...
- [ ] API tests for status transitions:
  ...
  - tie -> `PETITIONED_TIE`
  - unopposed -> `ACTIVE`
- [ ] Test that `Seat.isPetitioned` flips once and cannot be reset.
