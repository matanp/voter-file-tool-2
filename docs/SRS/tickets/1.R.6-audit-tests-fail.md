...
- [x] Ensure `AuditAction` is available in Jest when running audit tests (e.g. add to Prisma mock stubs if needed, or ensure `jest.requireActual` preserves it).
- [x] Default `pnpm --filter voter-file-tool test` passes including `auditLog.test.ts` and `auditLogImmutability.test.ts`.
- [x] No need to run migrations in test env if mock suffices; document if generated client is required for audit enums.
