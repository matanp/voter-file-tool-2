...
- [x] Wrap capacity check + seat assignment + membership write in a **single database transaction** for both `add` and `handleRequest` accept.
- [x] Use `SELECT ... FOR UPDATE` or equivalent to lock the committee/term row (or relevant Seat rows) during the critical section to prevent concurrent oversubscription.
- [x] Ensure `assignNextAvailableSeat` operates on consistent state within the transaction (no interleaving with another request).
- [x] Add concurrency test: fire N concurrent add/accept requests where N would exceed capacity; assert exactly `maxSeats` succeed and the rest receive capacity error.
- [x] Document transaction boundaries and any isolation level requirements.
...
The critical section for capacity and seat allocation is:

1. Lock target committee row (`SELECT ... FOR UPDATE`).
2. Ensure seat rows exist for `(committeeListId, termId)`.
3. Read active membership count for `(committeeListId, termId, status='ACTIVE')`.
4. Compute next seat from currently active memberships.
5. Write membership transition/create (`SUBMITTED -> ACTIVE`, `REJECTED`, or direct add).

This full sequence runs inside a single `prisma.$transaction` callback in:
...
- Postgres default `READ COMMITTED` plus explicit `FOR UPDATE` lock on `CommitteeList` row serializes concurrent seat-allocation writers for the same committee.
- Capacity checks and seat assignment must never run outside that lock-scoped transaction.
