...
#### Schema
- [x] Add `vacancyReportSchema` to `packages/shared-validators/src/schemas/report.ts`:
  ```ts
  ...
  ```
- [x] Add to union and `REPORT_TYPE_MAPPINGS`: `vacancyReport: { databaseValue: 'VacancyReport', filename: 'vacancyReport' }`.

#### Data Fetching
- [x] Create `fetchVacancyData()` in `committeeMappingHelpers.ts`:
...
#### PDF/XLSX Output
- [x] **PDF** (`VacancyReport.tsx`): Landscape table grouped by city/LD.
  - Columns: ED, Total Seats, Filled, Vacant, Petitioned (Filled/Vacant), Non-Petitioned.
  - Subtotal per LD group. Grand total at bottom.
- [x] **XLSX**: Single "Vacancy Report" worksheet with same columns + subtotal rows.

### B. Changes Report

#### Schema
- [x] Add `changesReportSchema`:
  ```ts
  ...
  ```
- [x] Add to union and mappings: `changesReport: { databaseValue: 'ChangesReport', filename: 'changesReport' }`.

#### Data Fetching
- [x] Create `fetchChangesData()` in `committeeMappingHelpers.ts`:
...
#### PDF/XLSX Output
- [x] **PDF** (`ChangesReport.tsx`): Portrait or landscape table.
...
- [x] **XLSX**: Single "Changes" worksheet with same columns + filters.

### C. Petition Outcomes Report

#### Schema
- [x] Add `petitionOutcomesReportSchema`:
  ```ts
  ...
  ```
- [x] Add to union and mappings: `petitionOutcomesReport: { databaseValue: 'PetitionOutcomesReport', filename: 'petitionOutcomesReport' }`.

#### Data Fetching
- [x] Create `fetchPetitionOutcomesData()` in `committeeMappingHelpers.ts`:
...
#### PDF/XLSX Output
- [x] **PDF** (`PetitionOutcomesReport.tsx`): Landscape table grouped by committee.
...
- [x] Each form has: name, format (PDF/XLSX), scope (jurisdiction/countywide), jurisdiction filters, type-specific params.
- [x] Enable all 3 cards in `GenerateReportGrid.tsx`.
- [x] Update `formatReportType()` in `reportUtils.ts` for all 3 types.
...
- [x] Publish a report parameter matrix as a table in this ticket and/or in `docs/SRS/` covering ALL report types:
...
### Testing
- [x] Report-server: Test each `fetch*Data()` function with scope filtering and edge cases (empty results, no petitioned seats).
- [x] Report-server: Test each PDF component renders correct structure.
- [x] Report-server: Test each handler in `processJob()`.
- [x] Frontend: Test form validation for each report type.
- [x] Frontend: Test Leader scope restrictions.
- [x] Integration: Generate one report of each type and verify output.
