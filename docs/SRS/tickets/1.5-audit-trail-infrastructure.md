...
| --- | --- | --- | --- |
| [1.5a](1.5a-audit-log-schema-and-seed.md) | AuditLog Schema & SYSTEM User Seed | Done | 1–2 days |
| [1.5b](1.5b-audit-log-utility-and-immutability.md) | Audit Log Utility & Immutability Guard | Done | 1–2 days |
| [1.5c](1.5c-audit-log-route-wiring.md) | Audit Log Route Wiring | Done | 1–2 days |
...
- [x] New `AuditAction` enum per [SRS_DATA_MODEL_CHANGES.md](../SRS_DATA_MODEL_CHANGES.md) §1:
...
- [x] New `AuditLog` model per [SRS_DATA_MODEL_CHANGES.md](../SRS_DATA_MODEL_CHANGES.md) §2:
...
- [x] Add `auditLogs AuditLog[] @relation("AuditLogs")` to `User` model (if not already added in 1.2)
- [x] **Immutability:** no Prisma `update` or `delete` operations on `AuditLog` anywhere in the codebase — records are write-once
- [x] **SYSTEM user seed:** add a `User` record with a fixed well-known `id: "system"` and placeholder email `system@internal` in `apps/frontend/prisma/seed.ts` (re-entrant: `upsert` with `where: { id: "system" }`). Required because `AuditLog.userId` is a non-nullable FK — a bare string sentinel would violate referential integrity

### Utility Function

- [x] Create `apps/frontend/src/lib/auditLog.ts` exporting:
...
- [x] Implementation: single `prisma.auditLog.create(...)` call — no batch, no buffering
- [x] Export a constant `SYSTEM_USER_ID = "system"` from `auditLog.ts`; callers pass this for system-initiated operations (e.g. scheduled jobs, migrations). Requires the SYSTEM user seed above
- [x] Do not throw on audit failure — catch errors and `console.error`; audit must never block the primary operation

### Route Wiring

Wire `logAuditEvent` calls into all existing mutation routes (after the primary DB operation succeeds):

- [x] `committee/add`: log `MEMBER_ACTIVATED` (admin direct-add creates an active record); include `beforeValue=null`, `afterValue=membership`
- [x] `committee/remove`: log `MEMBER_REMOVED`; include `beforeValue={status: previousStatus}`, `afterValue={status: "REMOVED", removalReason}`
- [x] `committee/handleRequest` (accept): log `MEMBER_ACTIVATED`; include transition snapshot
- [x] `committee/handleRequest` (reject): log `MEMBER_REJECTED`; include `rejectionNote` in `afterValue`
- [x] `committee/requestAdd`: log `MEMBER_SUBMITTED`; include `beforeValue=null`, `afterValue={status: "SUBMITTED"}`

### Testing

- [x] Unit tests for `logAuditEvent`:
  - Creates an `AuditLog` record with correct fields
  - Does not throw when Prisma create fails (logs to console.error)
- [x] Integration tests for one wired route (e.g. `committee/add`): verify `AuditLog` record is created after a successful add
