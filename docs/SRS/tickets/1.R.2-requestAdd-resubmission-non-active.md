...
- [x] When an existing `CommitteeMembership` exists with status in {REJECTED, REMOVED, RESIGNED, PETITIONED_*}, and the same voter/committee/term is submitted again:
...
- [x] Do not create a duplicate `CommitteeMembership`; always use update when a record exists for the unique key.
- [x] Add route test: resubmit for voter with REJECTED membership → succeeds, record transitions to SUBMITTED.
- [x] Add route test: resubmit for voter with REMOVED membership → succeeds.
