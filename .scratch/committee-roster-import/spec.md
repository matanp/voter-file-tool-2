...
committed fixture: a structurally faithful pseudonymized excerpt of the real file, roughly
twenty rows, preserving the original header row, delimiter, padding, cell types and layout
quirks verbatim while every person and VRCNUM in it is invented. Real voter data stays
outside Git; the fixtures are generated by `scripts/makeRosterFixtures.ts`. Tests assert the
canonical entries produced from them.
...
Prior analysis of this file, written against the tab-delimited `.txt` the Board of
Elections delivered, exists in the repo's docs as a format reconciliation note. Its
conclusions about committee identity, the absent Rochester special case and the unchanged
discrepancy fields hold. One correction: it recommends reading by header name, having caught
only the first two columns being mislabelled. The misalignment extends further — the declared
residential address columns do not correspond to the data either — so header-keyed reading
is unsafe regardless.

The delivered `.txt` and the locally converted `.csv` carry the same 52-field positional
layout; only the separator differs. Both were parsed against the current parser and each
yields 1,470 entries, zero rejections, 21 towns and only `PETITIONED` membership types. The
parser accepts either separator, chosen by which one decodes the header into the 52-field
layout; it does not sniff arbitrary delimiters and the Admin still states the `format`.
