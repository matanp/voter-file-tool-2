...
interface CommitteeRosterReportFormProps {
...
export function CommitteeRosterReportForm({
  committeeLists,
  userPrivilegeLevel,
}: CommitteeRosterReportFormProps) {
  ...
  const [name, setName] = useState(`Committee Roster - ${formatTodayDate()}`);
  ...
  >("/api/generateReport", "POST", {
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Your committee roster report is being generated.",
      });
    },
    onError: (error) => {
      toast({
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      {generateReportMutation.loading && (
        <div className="bg-primary-foreground p-4 rounded-lg flex items-center gap-2">
          ...
          <span>Generating committee roster...</span>
        </div>
      )}
      ...
      {reportId && (
        <ReportStatusTracker
          ...
          onComplete={(url) => {
            toast({
              description: "Committee roster report generated successfully!",
              duration: 5000,
            });
            setReportUrl(url);
            setReportId(null);
          ...
          onError={(message) => {
            toast({
              ...
              description: message || "Failed to generate committee roster",
              duration: 5000,
            });
            setReportId(null);
          }}
      ...
      {reportUrl && (
        <div className="space-y-4">
          <p className="font-medium">
            Committee roster report generated successfully!
          </p>
          <a
            href={reportUrl}
            target="_blank"
